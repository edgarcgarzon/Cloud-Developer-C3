...
export function requireAuth(req: Request, res: Response, next: NextFunction) {
  if (!req.headers || !req.headers.authorization) {
    log.error("auth", 'No authorization headers.');
    return res.status(401).send({message: 'No authorization headers.'});
  }

  const tokenBearer = req.headers.authorization.split(' ');
  if (tokenBearer.length != 2) {
    log.error("auth", 'Malformed token.');
  ...
  return jwt.verify(token, c.config.jwt.secret, (err, decoded) => {
    if (err) {
      log.error("auth", 'Failed to authenticate.');
...
router.post('/login', async (req: Request, res: Response) => {
  ...
  if (!email || !EmailValidator.validate(email)) {
    log.error("auth", 'Email is required or malformed.');
    return res.status(400).send({auth: false, message: 'Email is required or malformed.'});
  }

  if (!password) {
    log.error("auth", 'Password is required.');
    return res.status(400).send({auth: false, message: 'Password is required.'});
  }

  const user = await User.findByPk(email);
  if (!user) {
    log.error("auth", 'User was not found..');
  ...
  if (!authValid) {
    log.error("auth", 'Password was invalid.');
...
router.post('/', async (req: Request, res: Response) => {
  ...
  if (!email || !EmailValidator.validate(email)) {
    console.log('Email is missing or malformed.');
